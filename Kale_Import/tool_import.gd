...
func build_panel() -> Control:
	...
	_folder_input.placeholder_text = "Type path or browse to folder"
	_folder_input.text = DEFAULT_FOLDER
	_folder_input.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	folder_row.add_child(_folder_input)

	var browse_btn := Button.new()
	browse_btn.text = "Browse"
	browse_btn.pressed.connect(_on_browse_folder)
	folder_row.add_child(browse_btn)
	_panel.add_child(folder_row)
	...
	_folder_dialog.dir_selected.connect(func(path: String):
		_folder_input.text = path + "/"
	)
	_panel.add_child(_folder_dialog)
	...
	var local_tip := Label.new()
	local_tip.text = "Unpacks geometry from the referenced file into the current scene"
	local_tip.add_theme_font_size_override("font_size", 10)
	_panel.add_child(local_tip)
