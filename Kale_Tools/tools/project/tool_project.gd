...
const Presets = preload("project_data.gd")
...
func build_panel() -> Control:
	Presets.plugin_root = plugin_root
	...
	var tip := Label.new()
	tip.text = "Decals only project onto \"Static\" geometry.\nYou can spawn any map decal regardless of level,\nbut pre-configured transforms may not align\nif you pick the wrong map.\nIf nothing appears after spawning, check that\n\"Static\" geometry exists in your scene tree."
	tip.add_theme_font_size_override("font_size", 10)
	tip.clip_contents = true
...
func _on_map_selected(idx: int) -> void:
	...
	var raw = Presets.get_preset(name)
	...
	if raw is Array:
		...
		for entry in raw:
			...
			if copy.has("tex"):
				copy["tex"] = plugin_root + "/textures/" + copy["tex"]
			_preset_data.append(copy)
	elif raw is Dictionary:
		_preset_data = raw.duplicate(true)
		if _preset_data.has("tex"):
			_preset_data["tex"] = plugin_root + "/textures/" + _preset_data["tex"]
