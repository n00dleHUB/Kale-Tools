...
static func _get_cache_dir() -> String:
	return plugin_root + "/cache/"


static func _ensure_cache_dir() -> void:
	if not DirAccess.dir_exists_absolute(_get_cache_dir()):
		DirAccess.make_dir_recursive_absolute(_get_cache_dir())
	var gdignore := _get_cache_dir() + ".gdignore"
...
static func _get_cache_path(scene_root: Node) -> String:
	...
	return _get_cache_dir() + "chroma_" + hash_str + ".cfg"
...
static func clear_all() -> void:
	if not DirAccess.dir_exists_absolute(_get_cache_dir()):
		return
	var dir := DirAccess.open(_get_cache_dir())
	if not dir:
		return
	dir.list_dir_begin()
	var f := dir.get_next()
...
static func get_cache_dir_path() -> String:
	_ensure_cache_dir()
	return ProjectSettings.globalize_path(_get_cache_dir())
