...
func build_panel() -> Control:
	...
	for i in 5:
		...
		if saved_order[i] == 0:
			...
			_del_input.placeholder_text = "node_prefix"
			_del_input.text = "node_prefix"
...
func _find_tool(name: String) -> Node:
	...
	for child in dock.get_children():
		if child is Control and child.name == name and child.get_child_count() > 0:
			return child.get_child(child.get_child_count() - 1) as Node
	return null
