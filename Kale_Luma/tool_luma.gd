...
const Env = preload("luma_environment.gd")

var _panel: VBoxContainer
var _preset: OptionButton
var _hdri_list: OptionButton
var _custom_hdri_path: LineEdit
var _hdri_file_dialog: FileDialog
...
var _sun_shadows: CheckBox
var _sun_shadow_distance_spin: SpinBox
var _sun_shadow_distance_slider: HSlider
var _sun_shadow_bias_spin: SpinBox
var _sun_shadow_bias_slider: HSlider
...
func build_panel() -> Control:
	...
	_custom_hdri_path = LineEdit.new()
	_custom_hdri_path.placeholder_text = "Custom HDRI path (overrides dropdown)"
	_custom_hdri_path.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_custom_hdri_path.text_changed.connect(func(_t): _on_changed())
	custom_row.add_child(_custom_hdri_path)

	var hdri_browse := Button.new()
	hdri_browse.text = "Browse"
	hdri_browse.pressed.connect(_on_browse_hdri)
	custom_row.add_child(hdri_browse)

	var hdri_clear := Button.new()
	hdri_clear.text = "Clear"
	hdri_clear.pressed.connect(func():
		_custom_hdri_path.text = ""
		_on_changed()
	)
	custom_row.add_child(hdri_clear)
	_panel.add_child(custom_row)
	...
	_panel.add_child(_make_section("Ambient Light", false, amb_body))
	_panel.add_child(HSeparator.new())
	...
	sun_body.add_child(_make_slider_row("Elevation:", _sun_elevation_spin, _sun_elevation_slider, 0.0, 90.0, 0.1, 40.0))
	...
	# Shadows
	_sun_shadows = CheckBox.new()
	_sun_shadows.text = "Cast Shadows"
	_sun_shadows.button_pressed = true
	_sun_shadows.toggled.connect(func(_t: bool): _on_changed())
	sun_body.add_child(_sun_shadows)
	...
	_sun_shadow_distance_slider.value_changed.connect(func(v):
		if _setting_slider: return
		_setting_slider = true
		_sun_shadow_distance_spin.value = v
		_setting_slider = false
		_on_changed()
	)
	_sun_shadow_distance_spin.value_changed.connect(func(v):
		if _setting_slider: return
		_setting_slider = true
		_sun_shadow_distance_slider.value = v
		_setting_slider = false
		_on_changed()
	)
	...
	_sun_shadow_bias_slider.value_changed.connect(func(v):
		if _setting_slider: return
		_setting_slider = true
		_sun_shadow_bias_spin.value = v
		_setting_slider = false
		_on_changed()
	)
	_sun_shadow_bias_spin.value_changed.connect(func(v):
		if _setting_slider: return
		_setting_slider = true
		_sun_shadow_bias_slider.value = v
		_setting_slider = false
		_on_changed()
	)
	...
	for i in range(_preset.item_count):
		if _preset.get_item_text(i) == "Midday":
			neutral_idx = i
			break
	_preset.select(neutral_idx)
	_on_preset_changed(neutral_idx)

	_hdri_file_dialog = FileDialog.new()
	_hdri_file_dialog.file_mode = FileDialog.FILE_MODE_OPEN_FILE
	_hdri_file_dialog.add_filter("*.hdr,*.exr", "HDRI")
	_hdri_file_dialog.access = FileDialog.ACCESS_RESOURCES
	_hdri_file_dialog.file_selected.connect(func(path: String):
		_custom_hdri_path.text = path
		_on_changed()
	)
	_panel.add_child(_hdri_file_dialog)
...
func _on_preset_changed(idx: int) -> void:
	...
	var hdri_name: String = data.get("hdri", "")
	if not hdri_name.is_empty():
		for i in _hdri_list.item_count:
			if _hdri_list.get_item_text(i) == hdri_name:
				_hdri_list.select(i)
				break
	...
	_sun_elevation_slider.value = data.get("sun_elevation", 40.0)
	_sun_elevation_spin.value = data.get("sun_elevation", 40.0)
...
func _on_browse_hdri() -> void:
	_hdri_file_dialog.popup_centered(Vector2i(600, 400))
...
func _live_update() -> void:
	...
	if sky_on:
		var hdri := _custom_hdri_path.text.strip_edges()
		if hdri.is_empty() and _hdri_list.selected > 0:
			hdri = plugin_root + "/textures/" + _hdri_list.get_item_text(_hdri_list.selected)
	...
	Env.update_sun(
		...
		_sun_shadows.button_pressed,
		_sun_shadow_distance_spin.value,
		_sun_shadow_bias_spin.value,
	)
