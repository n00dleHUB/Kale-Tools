...
const Materials = preload("chroma_materials.gd")
const Cache = preload("chroma_cache.gd")
...
func build_panel() -> Control:
	Materials.plugin_root = plugin_root
	Cache.plugin_root = plugin_root
...
func _on_texture_list_changed(idx: int) -> void:
	if idx == 0:
		...
		_custom_tex_path.text = plugin_root + "/textures/" + fname
	_update_pattern_visibility()
...
func _update_texture_list_selection() -> void:
	...
	if path.is_empty() or not path.begins_with(plugin_root + "/textures/"):
		_texture_list.select(0)
		return
	var fname := path.trim_prefix(plugin_root + "/textures/")
